 ...
 *
 */
class codeScanner_logIndent extends baseExecuteTasks implements executeTasksInterface
{
        ...
        if ($isChanged)
        {
            if ($lastDepthCount != $newDepthCount)
            {
                ...
                if ($lastDepthCount < $newDepthCount)
                {
                    if ($lastDepthCount > -1)
                    {
                        print (str_repeat(' ', 3 * $lastDepthCount) . " { " . $actLineNumber . '/' . $newDepthCount . PHP_EOL);
                    }
                    else
                    {
                        print ($lastDepthCount . ":{ " . $actLineNumber . '/' . $newDepthCount . PHP_EOL);
                    }
                }
                else
                {
                    if ($newDepthCount > -1)
                    {
                        print (str_repeat(' ', 3 * $newDepthCount) . " } " . $actLineNumber . '/' . $newDepthCount . PHP_EOL);
                    }
                    else
                    {
                        print ($newDepthCount . ":{ " . $actLineNumber . '/' . $newDepthCount . PHP_EOL);
                    }
                }
